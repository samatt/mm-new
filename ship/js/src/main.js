...
var initShipping = function() {
	...
	getIcons = function( cb ) {
		...
		$.ajax( {
				method: "GET",
  				url: 'http://moreandmorestore.herokuapp.com/api/products/' + fromCountry.id + '/' + currentCountry.id,
...
var initForm = function() {
	...
	$doc.on( "submit", "#the-form", function( event ) {
		...
		$.post( "http://moreandmorestore.herokuapp.com/api/order", { name: name, email: email,from: fromCountry.id, where: currentCountry.id , product: currentItem.id })
          .done(function( data ) {
